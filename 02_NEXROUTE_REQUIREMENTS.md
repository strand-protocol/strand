# NexRoute — Layer 2: Semantic Routing & Intent-Based Addressing
...
NexRoute replaces traditional IP routing and BGP with a semantic, intent-based routing layer designed for AI workloads. Instead of routing packets to IP addresses (which identify network interfaces), NexRoute routes frames to **capabilities** — expressed as Semantic Address Descriptors (SADs). An AI agent can request "an LLM with code generation capability, 128K context, latency < 200ms" and NexRoute resolves and routes to the optimal endpoint in real-time.

NexRoute operates as a dataplane forwarding engine (in P4 for programmable switches, or in C for software dataplanes) paired with a control plane that maintains distributed capability routing tables via a gossip-based protocol.
...
|----------|-------|-----------|
| **RFC 791** | Internet Protocol (IPv4) | NexRoute replaces IPv4 addressing and forwarding with semantic addressing |
| **RFC 8200** | Internet Protocol, Version 6 (IPv6) | NexRoute's 128-bit Node IDs are analogous to IPv6 addresses but semantically derived |
| **RFC 4271** | A Border Gateway Protocol 4 (BGP-4) | NexRoute's capability advertisement protocol replaces BGP's path-vector routing with capability-vector routing |
| **RFC 4601** | Protocol Independent Multicast - Sparse Mode (PIM-SM) | Reference for multicast group management — NexRoute supports capability-based multicast groups |
| **RFC 1035** | Domain Names - Implementation and Specification (DNS) | NexRoute eliminates DNS for AI agent communication; semantic addressing replaces hostname resolution |
| **RFC 6830** | The Locator/ID Separation Protocol (LISP) | Architectural reference — LISP separates identity from location; NexRoute extends this to separate capability from location |
...
| **RFC 7938** | Use of BGP for Routing in Large-Scale Data Centers (BGP in DC) | Reference for datacenter routing patterns that NexRoute must coexist with |
| **RFC 4655** | A Path Computation Element (PCE) Architecture | Reference for centralized path computation — NexRoute's semantic resolver acts as a capability-aware PCE |
...
| `0x06` | `TRUST_LEVEL` | uint8 | Minimum NexTrust attestation level: 0=none, 1=identity, 2=provenance, 3=safety_eval, 4=full_audit |
...
│   └── nexlink.h                  # Imported from NexLink (C FFI header)
├── src/
...
│   ├── forwarding.c               # Software dataplane: NexLink frame → lookup → forward
...
├── p4/
│   ├── nexroute.p4                # P4_16 dataplane program for programmable switches
│   ├── headers.p4                 # NexLink + NexRoute header definitions in P4
│   ├── parser.p4                  # P4 parser for NexLink frames
...
├── config/
│   ├── nexroute.yaml              # Default configuration
│   └── sonic/
│       ├── nexroute.json          # SONiC ConfigDB schema for NexRoute
│       └── nexroute.service       # systemd service unit for SONiC
└── tools/
    ├── nexroute-cli.c             # CLI tool for inspecting routing tables
...
typedef struct {
    uint8_t node_id[16];           // NexLink Node ID
...
Weights are configurable per-deployment via nexroute.yaml.
```
...
| NR-S-002 | Decode binary SAD from NexLink frame options into `sad_t` struct | P0 |
...
| NR-G-005 | Gossip messages authenticated via NexTrust node identity (prevent route injection attacks) | P1 |
...
| NR-F-004 | Software forwarding engine: receive NexLink frame → extract SAD → resolve → rewrite dest_node_id → forward | P0 |
| NR-F-005 | Maintain per-path latency metrics via periodic probe frames (NexLink heartbeat type) | P1 |
...
| ID | Requirement | Priority |
|----|-------------|----------|
| NR-P4-001 | P4_16 parser for NexLink frame header (64-byte fixed header extraction) | P0 |
...
| ID | Requirement | Priority |
|----|-------------|----------|
| NR-SONIC-001 | Implement SAI (Switch Abstraction Interface) plugin for NexRoute table programming | P1 |
| NR-SONIC-002 | Register NexRoute as a SONiC Docker container with proper systemd service management | P1 |
| NR-SONIC-003 | Expose NexRoute configuration via SONiC ConfigDB (JSON schema) | P1 |
...
p4c --target bmv2 --arch v1model p4/nexroute.p4 -o build/nexroute.bmv2.json

# Build P4 for Tofino (requires Intel P4 Studio SDE)
# p4_build.sh p4/nexroute.p4 --target tofino2
...
| `NEXLINK_INCLUDE_DIR` | `../nexlink/include` | Path to NexLink C headers |
...
| P4 behavioral tests | BMv2 model: inject NexLink frames, verify SAD matching and forwarding | All P4 tables |
| Resolver accuracy tests | Pre-defined SAD queries with known-best answers, verify scoring correctness | All field types |
| Integration tests | NexLink frames from Zig → NexRoute C forwarding → NexLink output | Full datapath |
...
| NexLink C FFI | — | Frame encode/decode from NexLink module | Core |
