// Package transport defines the Transport interface used by NexAPI clients
...
// Transport is the abstract message-level transport used by the NexAPI client
// and server. Each call to Send/Recv operates on one complete NexAPI frame
// (opcode + payload). Implementations handle framing, buffering, and
// connection management internally.
type Transport interface {
	// Send transmits a single NexAPI frame identified by opcode with the
	// given payload. The context may carry deadlines or cancellation.
	Send(ctx context.Context, opcode byte, payload []byte) error

	// Recv blocks until a complete NexAPI frame arrives. It returns the
